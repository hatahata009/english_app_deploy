...
if "messages" not in st.session_state:
    ...
    # ★修正点1: ここでの chain_basic_conversation 作成は削除し、後ほどレベル確定後に作成します
...
def update_chains():
    # 日常英会話用Chain（レベルを埋め込む）
    system_template_basic = ct.SYSTEM_TEMPLATE_BASIC_CONVERSATION.format(level=st.session_state.englv)
    st.session_state.chain_basic_conversation = ft.create_chain(system_template_basic)

# 初回またはレベル変更時にChainを更新する必要があるが、
# 簡易的に「開始」時に最新のレベルでChainを作り直すロジックを以下に追加します。
...
if st.session_state.start_flg:
    ...
    # ★修正点3: 毎回最新のレベル設定でChainを更新・生成する
    # これにより、ユーザーがレベルを変更しても即座に反映されます
    if st.session_state.mode == ct.MODE_3 and st.session_state.dictation_first_flg:
        # ディクテーション用問題作成Chain
        system_template_prob = ct.SYSTEM_TEMPLATE_CREATE_PROBLEM.format(level=st.session_state.englv)
        st.session_state.chain_create_problem = ft.create_chain(system_template_prob)
    ...
    if st.session_state.mode == ct.MODE_1:
        ...
        # ★修正点4: 日常英会話Chainがない、または再生成が必要な場合
        # メモリは維持したいので、ChainのPromptだけ差し替えるのが理想ですが、
        # ここではシンプルにChainがない場合のみ作成し、レベル変更に対応するには
        # ページリロード推奨とするか、あるいはここで毎回再作成（メモリ引継ぎ）します。
        # 今回はシンプルに「まだChainがないなら作成」に加え「レベルを反映」させます。
        
        # 既存のChainがない場合は作成
        if not hasattr(st.session_state, 'chain_basic_conversation'):
            update_chains() # 初期作成
        ...
        with st.spinner('評価結果の生成中...'):
            if st.session_state.shadowing_evaluation_first_flg:
                system_template = ct.SYSTEM_TEMPLATE_EVALUATION.format(
                    ...
                    level=st.session_state.englv  # レベルを渡す
                )
                st.session_state.chain_evaluation = ft.create_chain(system_template)
